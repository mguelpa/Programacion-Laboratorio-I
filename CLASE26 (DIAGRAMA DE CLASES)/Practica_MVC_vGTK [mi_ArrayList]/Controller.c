...
{
    Socio* auxSocio;
    auxSocio = soc_new(nombre, apellido, dni, getNewIdSocios(), SOCIO_ACTIVO);
...
int ctrlr_bajaSocio (int id)
{
    ...
    if(auxSocio != NULL)
    {
        soc_setEstado (auxSocio, SOCIO_INACTIVO);
        dm_saveAllSocios (listaSocios);
    }
    return 0;
}
...
int ctrlr_checkSocio(int id)
{
    ...
    if(auxiliar != NULL && soc_getEstado(auxiliar) == SOCIO_ACTIVO)
    {
        retorno = 0;
    }
    return retorno;
...
{
    intr_mostrarSocios (listaSocios);
    return 0;
}
