...
static void opcionListado()
{
    ...
    ctrlr_listarSubjects();
    ctrlr_listarInscripciones();
}
void intr_mostrarSubjects(ArrayList* lista)
{
    int i;
    Subject* auxiliar;
    ...
    for(i=0; i<al_len(lista); i++)
    {
        ...
        if(auxiliar->subStatus == SUB_ACTIVE)
        {
            auxiliar = al_get(lista, i);

            printf("   %d\t", sub_getID(auxiliar));
            printf(" %s \t\t",sub_getName(auxiliar));
            printf(" %s \t\t", sub_getSurname(auxiliar));
            //printf(" %s\t", sub_getDni(auxiliar));
            printf("%d\n", sub_getStatus(auxiliar));
...
void intr_mostrarServicios (ArrayList* list)
{
    ...
    for(i=0; i<al_len(list); i++)
    {
        ...
        if(auxiliar->serStatus == SERVICIO_ACTIVO)
        {
            auxiliar = al_get(list, i);
...
static void opcionAltaInscripcion()
{
    ...
    {
        if( ctrlr_checkSubject(atoi(memberID)) == 0)
        {
            ctrlr_listarServicios();
...
//                {
                    if(val_getUnsignedInt(auxEstado,"Estado?\nACTIVO= 0\nINACTIVO= 1", "Error",2,5)==0)
                    {
                        ...
                        {
                            ctrlr_modificarSubject(id, estado, auxNombre, auxApellido);
                        }
                    }
//                }
            }
        ...
        if(val_getString (descripcion, "titulo? ", "Error",2,50)==0)
        {
            if(val_getString (descripcion, "Descripcion? ", "Error",2,50)==0)
            {
                if(val_getUnsignedInt (estado,"Estado?\nACTIVO= 0\nINACTIVO = 1", "Error",2,5)==0)
                {
                    ...
                    {
                        ctrlr_modificarServicio(atoi(id), atoi(estado), titulo, descripcion);
                    }
                }
            }
        }
...
static void opcionModificarInscripcion()
{
    ...
    {
        if( ctrlr_checkSubject (atoi(id)) == 0)
        {
            ctrlr_listarInscripciones();
