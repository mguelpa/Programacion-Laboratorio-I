...
void venta_imprimir (void* pVenta)
{
    char herramientas[5][50] = {"empty", "Taladro Neumatico", "Soldador MIG/TIG", "Martillo Neumatico", "Cierra de banco"};
    char estados[2][50] = {"VENTA_ACTIVA", "VENTA_FINALIZADA"};

        printf("ID Cliente: %d\n", venta_getClientID(pVenta));
        printf("ID venta: %d - ", venta_getVentaID(pVenta));
        printf("Estado venta: %s\n", estados[venta_getVentaStatus(pVenta)]);
        printf("ID producto %s\n", herramientas[venta_getProductID(pVenta)]);
        printf("Cantidad: %d\n\n", venta_getCantidad(pVenta));
        //printf("--------------------------------------\n\n");
}
